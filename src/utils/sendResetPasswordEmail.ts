...
export async function sendResetPasswordEmail(recipientEmail: string, userId: number) {
    const recipientIdJwt = jwt.sign(userId, process.env.JWT_SECRET);
    ...
    <body>
        <h1>Your password is ready to be reset!</h1>
        <main>
            <a href=${process.env.API_URL+"/users/reset-password-email?recipient="+recipientIdJwt}>
                <button class="accept">Reset Password</button>
            </a>
        </main>
    </body>
    </html>
